...
----------------------------------------------------------------------------------------------------------------------------
-- DARLEHEN - KUNDEN ÜBERSICHT NACH DARLEHEN 
-----------------------------------------------------------------------------------------------------------------------------
...
LEFT JOIN kunden_konten a 
    ON c.customer_id = a.customer_id
ORDER BY kunden_name, gesamt_darlehen_pro_kunde;

select min(credit_score), max(credit_score) from customers;
...
-- QUARTAL WACHSTUM
CREATE OR REPLACE VIEW Quarterly_transaction_growth AS
WITH quarterly_transaction AS (
    SELECT 
        TO_CHAR(transaction_date, 'YYYY-"Q"Q') AS quarter,
        COUNT(transaction_id) AS transaction_count,
